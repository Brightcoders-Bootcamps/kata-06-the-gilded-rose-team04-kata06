...
class Rules
  ...
  def get_type(item)
    ...
    special_products.select { |product| product.include? item }[0].class
  end
  ...
  def aged_brie_quality_calculator(item, _type)
  ...
  def sulfuras_quality_calculator(item, _type)
    item.quality = 80
    item.sell_in = -1
  end
  ...
  def normal_quality_calculator(item, _type)
